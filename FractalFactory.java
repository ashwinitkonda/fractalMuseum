...
public class FractalFactory {
    ...
    /**
     * Method takes in a pane object and draws a square fractal that is centerd on
     * the pane's preffered width and height. The base of the fractal will have a
     * size between 30 and 150 and a reduction factor between .05 and .55.
     *
    ...
    public static int drawFractal(Pane pane) {
        ...
        return drawFractal(0, size, (int) ((pane.getPrefWidth() / 2) - (size / 2)),
                (int) ((pane.getPrefHeight() / 2) - (size / 2)), 0.5 * Math.random() + 0.05, pane);
    }

    /**
     * Helper function which utilizes recursion to draw a fractal. Each recursive
     * call creates a subsection of the fractal at a different corner of the current
     * rectangle with 95% probability.
     *
    ...
    private static int drawFractal(int corner, int size, int x, int y, double factor, Pane pane) {
        if (size < 1) {
            ...
            Paint randomColor = Color.rgb(r, g, b);
